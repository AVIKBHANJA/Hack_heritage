...
export default function ProductPage() {
        ...
        <div className="p-8 grid gap-4">
          <div>
            ...
            <button
              ...
              className="fixed right-12 top-8 flex gap-1 py-2 px-4 rounded-2xl bg-amber-200 hover:bg-amber-400 active:bg-amber-100 text-white shadow shadow-gray-500"
            >
              Close Photos
            </button>
          </div>
  ...
  return (
    <>
      <h1 className="text-4xl flex font-bold bg-blue-200 m-4 p-2 rounded-full">
        {product.title}
      </h1>
      ...
      <div>
        <div className="flex flex-col items-center justify-center">
          <a
            ...
            target="_blank"
            href={"https://maps.google.com/?q=" + product.owneraddress}
          >
            ...
            <div className="object-cover w-80">
              {product.photos?.[2] && (
                <img
                  ...
                  className="aspect-square relative top-2 cursor-pointer"
                  src={"http://localhost:4000/uploads/" + product.photos[2]}
                  alt=""
                />
              )}
            </div>
            <div className="object-cover w-80">
              {product.photos?.[3] && (
                <img
                  onClick={() => setShowAllPhotos(true)}
                  className="aspect-square relative top-2 cursor-pointer"
                  src={"http://localhost:4000/uploads/" + product.photos[3]}
                  alt=""
                />
              )}
            </div>
            <div className="object-cover w-80">
              {product.photos?.[4] && (
                <img
                  onClick={() => setShowAllPhotos(true)}
                  className="aspect-square relative top-2 cursor-pointer"
                  src={"http://localhost:4000/uploads/" + product.photos[4]}
                  alt=""
                />
              )}
            </div>
          ...
          <br />
          <div className="flex gap-4 my-4 w-1/2 rounded-3xl overflow-hidden">
            <h1 className="font-bold text-2xl underline my-4">
              Prove of Authentication:
            </h1>
            <div className="object-cover w-80">
              {product.photos?.[0] && (
                <img
        ...
        <div>
          <div className="my-4 flex justify-center items-center flex-col">
            ...
            <p className="w-2/3">{product.history}</p>
            <h2 className="font-semibold text-3xl  ">Artist Authetic Proof</h2>
            <p className="w-2/3">{product.artistauth}</p>
            ...
            <div className="">
              ...
              <b>
                {product.perks.length > 0 && (
                  <ul>
                    <li>{product.perks[0]}</li>
                    <li>{product.perks[1]}</li>
                    <li>{product.perks[2]}</li>
                  </ul>
                )}
              </b>
              <div className="mt-2 mb-4 text-sm text-gray-700 leading-8"></div>
            </div>
